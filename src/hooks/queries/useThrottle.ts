// 주어진 값(상태)가 자주 변경될 떄 최소 interval 간격으로만 업데이트해서 성능을 개선함

import { useEffect, useRef, useState } from "react";

function useThrottle<T>(value: T, delay: number = 500): T {
  // 1. 상태 변수 : throttledValue : 최종적으로 쓰로틀링 적용된 값 줬아.
  // 초기값을 전달받은 value
  ...
  useEffect(() => {
    ...
    if (Date.now() >= lastExecuted.current + delay) {
      ...
      // Clean up 로직
      // 타이머가 아직 만료되지 않았는데 value나 delay가 변경되면, 이전 타이머를 취소해서 불필요한 업데이트 방지
